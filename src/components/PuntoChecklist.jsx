...
export function PuntoChecklist({ item, indice, resultado, onActualizar }) {
  ...
  return (
    <div className={"rounded-xl border bg-white p-4 shadow-sm " + (koSinComentario ? "border-amber-300" : "border-slate-200")}>
      <div className="flex items-start gap-3">
        ...
        <p className="flex-1 font-medium text-slate-800">{item.texto}</p>
      </div>
        ...
        {ESTADOS.map((est) => {
          ...
          return (
            <button key={est.valor} onClick={() => onActualizar({ ...resultado, estado: est.valor })}
              className={"flex-1 rounded-lg py-2.5 text-sm font-bold transition " + (activo ? est.clase + " shadow" : "bg-slate-100 text-slate-500 hover:bg-slate-200")}>
              {est.etiqueta}
            </button>
          );
        })}
      ...
      <input type="text" value={resultado.comentario} onChange={(e) => onActualizar({ ...resultado, comentario: e.target.value })}
        ...
        className={"mt-3 w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-orange-400 " + (koSinComentario ? "border-amber-300 bg-amber-50" : "border-slate-200")} />
      <div className="mt-3 flex items-center gap-3">
        <label className="cursor-pointer rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-200">
